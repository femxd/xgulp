...
_.touch = function (path, mtime) {
    ...
    if (mtime instanceof Date) {
        ...
        xgulp.log.error('invalid argument [mtime]');
    }
...
function getFileTypeReg(type) {
    ...
    if (type === 'text') {
        ...
        xgulp.log.error('invalid file type [%s]', type);
    }
    if (ext && ext.length) {
...
_.read = function (path, convert) {
    ...
    if (_exists(path)) {
        ...
        xgulp.log.error('unable to read file[%s]: No such file or directory.', path);
    }
    return content;
};
    ...
    function normalize(pattern) {
        ...
        switch (type) {
            ...
            case '[object RegExp]':
                return pattern;
            default:
                xgulp.log.error('invalid regexp [%s].', pattern);
        }
    }
...
_.find = function (rPath, include, exclude, root) {
    ...
    if (path) {
        ...
        xgulp.log.error('unable to find [%s]: No such file or directory.', rPath);
    }
    return list.sort();
};
...
_.del = function (rPath, include, exclude) {
    ...
    if (rPath && _.exists(rPath)) {
        ...
        if (/^(?:\w:)?\/$/.test(path)) {
            xgulp.log.error('unable to delete directory [%s].', rPath);
        }
        ...
        //xgulp.log.error('unable to delete [' + rPath + ']: No such file or directory.');
    }
    return removedAll;
};
...
_.copy = function (rSource, target, include, exclude, uncover, move) {
    ...
    if (source) {
        ...
        xgulp.log.error('unable to copy [%s]: No such file or directory.', rSource);
    }
    return removedAll;
};
...
// 若type为多层级(ex: 'a.b')，获取fis.config中groups[defaultGroup]['modules']下一层配置项的名称
        ...
        processors.forEach(function (obj, index) {
            ...
            if (typeof processor === 'function') {
                ...
                xgulp.log.warning('invalid processor [modules.' + key + ']');
            }
        });
    }
};
    ...
    var writer = fs.createWriteStream(tmp),
        http_err_handler = function (err) {
            ...
            if (callback) {
                ...
                xgulp.log.error('request error [%s]', msg);
            }
        },
                            ...
                            fs
                                ...
                                .on('error', function (err) {
                                    if (callback) {
                                        ...
                                        xgulp.log.error('extract tar file [%s] fail, error [%s]', tmp, err);
                                    }
                                })
                                .on('end', function () {
...
_.upload = function (url, opt, data, content, subpath, callback) {
    if (typeof content === 'string') {
        ...
        xgulp.log.error('unable to upload content [%s]', (typeof content));
    }
    data = data || {};
    var endl = '\r\n';
    ...
    _.download(url, function (err) {
        if (err) {
            if (opt.error) {
                ...
                xgulp.log.error('unable to download component [%s@%s] from [%s], error [%s].', name, version, url, err);
            }
        } else {
            if (opt.done) {
...
_.readJSON = function (path) {
    ...
    try {
        ...
        xgulp.log.error('parse json file[%s] fail, error [%s]', path, e.message);
    }
    return result;
};
...
_.nohup = function (cmd, options, callback) {
    ...
    if (IS_WIN) {
        ...
        return exec('cscript.exe /nologo "' + file + '"', options, function (error, stdout) {
            if (stdout != '0') {
                xgulp.log.error('exec command [%s] fail.', cmd);
            }
            fs.unlinkSync(file);
        ...
        return exec('nohup ' + cmd + ' > /dev/null 2>&1 &', options, function (error, stdout) {
            if (error !== null) {
                xgulp.log.error('exec command [%s] fail, stdout [%s].', cmd, stdout);
            }
            if (typeof callback === 'function') {
                callback();
            }
